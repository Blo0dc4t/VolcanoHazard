...
def stitch_series_with_eruption_drops(
    times,
    values,
    eruption_log,
    *,
    ...
    for eruption in sorted(eruption_log, key=lambda e: e["time_years"]):
        t = float(eruption["time_years"])
        before = float(eruption[before_key])
        after = float(eruption[after_key])

        # Find all points already at this time
        idxs = [i for i, tt in enumerate(times) if np.isclose(tt, t, atol=atol)]

        existing = [values[i] for i in idxs]

        has_before = any(np.isclose(v, before, atol=atol) for v in existing)
        has_after = any(np.isclose(v, after, atol=atol) for v in existing)

        # First eruption point at/after this time
        insert_idx = np.searchsorted(times, t)

        if not has_before:
            times.insert(insert_idx, t)
            values.insert(insert_idx, before)
            insert_idx += 1

        if not has_after:
            times.insert(insert_idx, t)
            values.insert(insert_idx, after)
